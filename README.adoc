...
== Systemarchitektur
Grundsätzlich wird die Applikation auf einem NGINX-Server gehostet der unter http://vm85.htl-leonding.ac.at/tadeot/ erreichbar ist. Die Anwendung basiert auf HTML und JS somit läuft sie auf allen gängigen Browsern. Wird nach einem Lehrer gesucht und gefunden wird eine MQTT-Anfrage an den Broker geschickt der dann eine Route empfängt zum einzeichnen. Weitere Abläufe entnehmen Sie bitte der Abbildung.
